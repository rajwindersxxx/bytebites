...
interface props {
  ...
  updateFunction: (variables: {ingredientId: number, purchasedStatus: boolean}) => void;
}
function PendingCardListItem({ ingredientObject,updateFunction }: props) {
  ...
  return (
    <li className="grid grid-cols-[auto_1fr_0.6fr_auto] items-center gap-2 rounded-full bg-natural-cream p-1">
      ...
      <button className="h-7 w-7 rounded-full transition-colors hover:bg-accent" onClick={() => updateFunction({ingredientId: Number(id),purchasedStatus: true})} >
        ✔
      </button>
    </li>
  );
