...
export function makeAQuestion(ingredients: string | null) {
  ...
  OUTPUT: singleString (Output should not include any square bracket),
      ...
      .map((item) => {
        ...
        const object = {
          ...
          ingredients: data[3]
            ...
            .map((item) => ({ id: Math.random(), name: item.trim() })),
          equipment: data[4]
            .split(",")
            .filter((item) => item.trim())
            .map((item) => ({ id: Math.random(), name: item.trim() })),
